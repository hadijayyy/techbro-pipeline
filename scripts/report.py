...
# Read token from Hermes .env (@Szejay_bot)
BOT_TOKEN = ""
...
def send_report(info):
    if not BOT_TOKEN:
        print("No TELEGRAM_BOT_TOKEN, skipping report")
        return
    ...
    if info["posted"]:
        ...
        emoji = "⏳"
        status = "STAGED (pending post)"
    elif info["error"]:
        emoji = "❌"
        status = f"FAILED: {info['error']}"
    else:
        emoji = "⏭️"
        status = "SKIPPED (no articles)"
    
    lines = [
        f"{emoji} **TechBro Report** — {info['time']}",
        f"Status: {status}",
    ]
    ...
    if r.status_code == 200:
        print(f"Report sent to Telegram thread {THREAD_ID}")
    else:
